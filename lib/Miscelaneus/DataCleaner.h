...
class DataCleaner {
  ...
  public:
    DataCleaner();
    ~DataCleaner(); // Destructor to clean up when class instance killed
...
// Constructor
template <typename T>
DataCleaner<T>::DataCleaner () { // Constructor
  
}

// Destructor
template <typename T>
DataCleaner<T>::~DataCleaner () { // Destructor
  delete[] smoothReading;
}

// Inintialise the array for storing sensor values
template <typename T>
bool DataCleaner<T>::begin (byte mode, byte smoothFactor) { 
  smoothMode = mode;
...
template <typename T>
bool DataCleaner<T>::add (T newReading) {
  switch (smoothMode) {    
    case SMOOTHED_AVERAGE : // SMOOTHED_AVERAGE
