...
struct PCF8591::AnalogInput PCF8591::analogReadAll(byte readType) {
  ...
  byte operation =
      AUTOINCREMENT_READ | readType | (_outputStatus & OUTPUT_MASK);
 ...
 * @param microcontrollerReferenceVoltage get voltage from microcontroller
 * voltage (only AVR no esp8266 for esp 3.3v fixed)
 * @param referenceVoltage if microcontrollerReferenceVoltage false take this
 * value
 * @return
 */
float PCF8591::voltageRead(uint8_t analogPin,
                           bool microcontrollerReferenceVoltage,
                           float referenceVoltage) {
  float voltageRef = referenceVoltage;
  if (microcontrollerReferenceVoltage)
    voltageRef = PCF8591::readVcc() / 1000.0;
 ...
 * value
 */
void PCF8591::voltageWrite(float value, bool microcontrollerReferenceVoltage,
                           float referenceVoltage) {
  if (microcontrollerReferenceVoltage)
    referenceVoltage = PCF8591::readVcc() / 1000.0;
